...
Using 10 fold cross-validation the models gave a lower RMSE and a reasonable R squared. After using cross-validation the explanatory power of the models was below the 0.92 that we had originally used the conventional multivariate linear regression and stepwise regression. The stepwise regression selected 46 variables out of the 72 variables we had after performing data cleaning and processing. However, the R-squared values were considered high  0.92. The calculated RMSE value was high once we run the conventional regression. The linear regression using 10 fold cross-validation gave a Rsquare of 0.81 and a RMSE of 0.17  whereas Random Forest gave a Rsquareof 0.87 and RMSE of 0.145. The Cross-validation allowed us to maximize the value of the limited data points and later to avoid overfitting the models.
