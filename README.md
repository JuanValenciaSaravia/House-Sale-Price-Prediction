...
Home value predictions are used by realtors, mortgage lenders, and financial institutions to define loan types and rates. Predictions are also used by government agencies to overview the market trends, define policies to predict and mitigate potential market bubbles that could affect negatively the housing market.
The purpose of the project is to predict home values based on the characteristics of the property. Using a multivariate regression,and other machine learning algorithms we will create a model that will predict the values.
...
Using 10 fold cross validation the models gave a lower RMSE and a reasonable Rsquare. After using cross validation the 
explanatory power of the models were below the 0.92 that we had originally using the conventional multivariate linear 		
regression and stepwise regression.The stepwise regression selected 46 variables out of the 72 variables we had after 		
performing data cleaning and processing.However,the Rsquare values were considered still high  0.92. The calculated RMSE value was high once we run the conventinal regression.The linear regression using 10 fold cross validation gave a Rsquare of 0.81 and a RMSE of 0.17  whereas Random Forest gave a Rsquareof 0.87 and RMSE of 0.145. The Cross validation allowed us to maximize the value of the limited data points and later to avoid overfitting the models.
